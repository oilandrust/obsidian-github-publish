...
const OUT = path.join(ROOT, 'assets', 'toolchain-quartz');
  ...
  build:
    ...
    steps:
      ...
      - name: Overlay user site
        run: |
          # Point Quartz at the site checkout's content via -d so
          # CreatedModifiedDate discovers the wiki git history (not the engine's).
